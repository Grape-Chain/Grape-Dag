...
func TestProtobuffTxCreationSerializationSigningAndVerification(t *testing.T) {
	...
	// Sign unsigned bytes of transaction via luna1crypto API using sender's Private Key
	signature := luna1crypto.NewDSA().Sign(*wallet.PrivateKey(), unsignedBytes)
	...
	var verified = luna1crypto.NewDSA().Verify(deserializedSignedTx.SenderPubk, deserializedSignedTx.Signature, serializedPbBytesToVerify)
	if !verified {
