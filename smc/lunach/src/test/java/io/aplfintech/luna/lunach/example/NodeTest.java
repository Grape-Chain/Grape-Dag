...
class NodeTest {
    ...
    @Disabled("Requires the started external Luna1Node server")
    @Test
    void transfer() {
    ...
    void callContract() {
        ...
        var contractAddress = Lunach.createAddress(HexUtils.parseHex(genesisAddrHex), account.nonce - 1);
    ...
    void estimateCallContract() {
        ...
        var account = retrieveAccount(genesisAddrHex);
        var contractAddress = Lunach.createAddress(HexUtils.parseHex(genesisAddrHex), account.nonce - 2);
