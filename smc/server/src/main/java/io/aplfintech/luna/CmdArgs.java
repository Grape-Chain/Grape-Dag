...
public class CmdArgs {
    @CommandLine.Option(names = {"--state-server-port", "-ssp"}, description = "Port of the state server where from fetch accounts, mappings, etc (Typically - a Luna1 peer's state grpc port)", defaultValue = "39399")
    Integer stateServerPort;
    @CommandLine.Option(names = {"--state-server-host", "-ssh"}, description = "Host of the state server where from fetch accounts, mappings, etc (Typically - a Luna1 peer's state grpc port)", defaultValue = "localhost")
